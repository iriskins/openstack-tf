...
resource "openstack_compute_instance_v2" "postgresql01_instance" {
  region = "RegionOne"
  name            = "postgresql01.iriskin.edu"
  image_id        = openstack_images_image_v2.ubuntu-plucky-server.image_id
  flavor_id       = openstack_compute_flavor_v2.cherry.flavor_id
  security_groups = [openstack_networking_secgroup_v2.all.name]
  availability_zone = "nova"
  ...
  user_data = "#cloud-config\nhostname: postgresql01.iriskin.edu\nfqdn: postgresql01.iriskin.edu\npackage_upgrade: true\npackages:\n  - vim"
  power_state = "active"
  hypervisor_hostname = "cc01.iriskin.edu"
  depends_on = [
    openstack_networking_network_v2.selfservice,
